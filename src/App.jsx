...
setDay(validDay);setLogs(l);setCari(c||[]);setEc(finalEc);setOnlineOrders(onl);setInstallments(inst||[]);setUnlocked(unl);setNotifications(notif);setTodos(td_);setTbl(t||[]);
...
<div style={{fontWeight:600,fontSize:15,marginBottom:30}}>Son 7 Gün</div>
<div style={{display:"flex",gap:6,alignItems:"flex-end",height:70,position:"relative"}}>
{l7.map((d,i)=>{const h=d.inc>0?Math.max((d.inc/mx)*60,6):2;const isT=d.date===tod();return(<div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",height:"100%",justifyContent:"flex-end",position:"relative"}}>{d.inc>0&&<div style={{position:"absolute",bottom:h+8,fontSize:9,color:T.textSub,textAlign:"center",whiteSpace:"nowrap",fontWeight:600}}>{fm(d.inc,cur).replace(cur,"").trim()}</div>}<div style={{width:"100%",height:h,background:isT?"#34C759":"rgba(52,199,89,0.2)",borderRadius:"4px 4px 0 0"}}/><div style={{fontSize:10,color:isT?"#34C759":T.textSub,fontWeight:isT?700:400,marginTop:4}}>{d.lbl}</div></div>);})}
</div>
</div>
</div>
